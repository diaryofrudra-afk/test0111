...
const CONFIG = {
  ...
  url: process.env.BLACKBUCK_URL || 'https://blackbuck.com/boss/gps',
...
async function fetchFromBlackbuck() {
  ...
  try {
    ...
    const needsLogin = currentUrl.includes('login') ||
      ...
      await page.evaluate(() => document.body.innerText.includes('Login With OTP'));

    if (needsLogin) {
      log('info', 'Login required, entering credentials…');

      // ── Handle "Login with password" toggle ──
      const passLink = await page.waitForSelector('button:has-text("Login with password")', { timeout: 3000 }).catch(() => null);
      if (passLink) {
        log('debug', 'Switching to password login mode…');
...
function normalizeVehicle(raw) {
  ...
  const reg = String(find('reg', 'vehicle_no', 'vehicleno', 'number_plate', 'registration', 'rc_no', 'truck_no') || '')
  ...
  const ignition = find('ignition', 'ign', 'engine_status', 'ignition_status');
  const odometer = parseFloat(find('odometer', 'odo', 'total_distance', 'km_driven', 'travelled_today')) || null;
  ...
  const lastUpdate = find('last_update', 'updated_at', 'timestamp', 'gps_time', 'last_seen', 'last_updated_on');
